...
//        //User user=new User();
        ...
        String data=request.getParameter("data");//获取移动端传来的密码
        
        System.out.println("数据*************"+data);
        String data1=new String(data.getBytes("ISO-8859-1"),"utf-8");
        JSONObject object=JSONObject.fromObject(data1);
        
        String account=object.getString("U_account").toString();
        String password=object.getString("U_password").toString();
...
//        //user=impl.userSelfSearch(1);
        ...
        if(exsit.equals("exsit")){
        	String data2=impl.userSelfSearch(account);
        	System.out.println("shuju*-*-*-*-"+data2);
        	JSONArray array=JSONArray.fromObject(data2);
        	
        	JSONObject object2=(JSONObject) array.get(0);
        	object2.put("U_headP", "success");
